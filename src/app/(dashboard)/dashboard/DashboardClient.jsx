...
import { Users, FolderOpen, Bell, ArrowRight, Plus, Check, X, Merge, AlertTriangle, UserPlus, Mail } from 'lucide-react';
...
export default function DashboardClient() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch pending team invites (invites sent TO this user)
  ...
  // Handle team invite actions (accept/reject)
  const handleInviteAction = async (invite, action) => {
    setActionLoading(true);
    try {
      const projectTitle = invite.teams?.projects?.title || 'a project';
      const teamOwnerName = invite.teams?.profiles?.full_name || 'Team Owner';
      const teamOwnerId = invite.teams?.owner_id;
      const teamOwnerEmail = invite.teams?.profiles?.email;
      ...
      if (action === 'accepted') {
          ...
          try {
            await fetch('/api/email', {
              ...
              body: JSON.stringify({
                ...
                recipientName: teamOwnerName,
                actorName: profile?.full_name,
                projectName: projectTitle,
              }),
            });
          } catch (emailErr) {
            console.error('Failed to send invite accepted email:', emailErr);
          }
        }

        showToast({ title: 'Invitation accepted', message: `You are now a member of the team for ${projectTitle}.`, variant: 'success' });
      } else {
        // Notify team owner of rejection
        if (teamOwnerId) {
          await supabase.from('notifications').insert({
            user_id: teamOwnerId,
        ...
        if (teamOwnerEmail) {
          try {
            ...
            console.error('Failed to send invite rejected email:', emailErr);
          }
        }
  ...
  return (
    <div className={styles.dashboard}>
      ...
      {/* Pending Team Invites (received by user) */}
      {pendingInvites.length > 0 && (
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            <h2 className={styles.sectionTitle}>
              Team Invitations
              <Badge variant="success" size="sm" style={{ marginLeft: 'var(--space-sm)' }}>{pendingInvites.length}</Badge>
            </h2>
          </div>
          <div className={styles.requestList}>
              ...
              <Card key={inv.id} className={styles.inviteCard}>
                <div className={styles.inviteCardContent}>
                  <div className={styles.inviteCardInfo}>
                    <Avatar
                      name={inv.teams?.profiles?.full_name}
                      src={inv.teams?.profiles?.avatar_url}
                      size="sm"
                    />
                    <div>
                      <p className={styles.requestName}>
                        <Link href={`/profile/${inv.teams?.profiles?.id}`} style={{ color: 'var(--color-primary)', textDecoration: 'none' }} title="View Profile">
                          {inv.teams?.profiles?.full_name}
                        </Link>
                      </p>
                      <p className={styles.requestProject}>invited you to join their team for {inv.teams?.projects?.title}</p>
                      {inv.message && <p className={styles.requestMessage}>&ldquo;{inv.message}&rdquo;</p>}
                    </div>
                  </div>
                  <div className={styles.inviteActions}>
                    ...
                    <button
                      ...
                      onClick={() => handleInviteAction(inv, 'rejected')}
                      disabled={actionLoading}
                      title="Decline"
                    >
                      <X size={16} /> Decline
